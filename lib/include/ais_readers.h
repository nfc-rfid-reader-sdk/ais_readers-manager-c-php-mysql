 ...
 *
 * @param device
 * @param records_to_ack 0 or 1 : ACK only one; 2 and more : ACK multiple unread LOGs
 * @return
 */
DL_API
 ...
 * @param auto_stop seconds of the signal duration.
 * 			Signal stops after provided time in seconds or if digital input signal from the turnstile gate (passage) occurred.
 * 			If provide 0 - infinity, auto stop signal mechanism is not used.
 *
 * @return DL_STATUS
 */
DL_API
DL_STATUS AIS_SignalSet(HND_AIS device, uint32_t sound, uint32_t light,
		uint32_t output, uint32_t auto_stop);
...
/**
 * Safely upload firmware to the device from buffer and flash
 *
 * Blocking function.
 *
 * @param device
 * @param password
 * @param fw_data [IN] data array with firmware's binaries (eg. data from BaseHD_FWv240.bin)
 * @param fw_size [IN] firmware length
 * @return
 */
DL_API
DL_STATUS AIS_FW_Upload(HND_AIS device, c_string password, uint8_t *fw_data,
		uint32_t fw_size);

/**
 * Safely upload firmware to the device from file and flash
 *
 * Blocking function.
 *
 * @param device
 * @param password
 * @param fw_bin_filename [IN] path to firmware binaries (eg. "BaseHD_FWv240.bin")
 * @return
 */
DL_API
DL_STATUS AIS_FW_Upload_File(HND_AIS device, c_string password,
		c_string fw_bin_filename);
